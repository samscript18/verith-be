...
| `VERTEX_AUTHENTICATION_FAILED`          |         503 | Google ADC or the configured Vertex service account could not authenticate.                   |
| `VERTEX_INVALID_REQUEST`                |         503 | Vertex rejected the adapter payload; the same credential is not retried.                      |
| `VERTEX_EMPTY_RESPONSE`                 |         503 | Vertex accepted the operation but returned no presentation content.                           |
| `VERTEX_CONTENT_BLOCKED`                |         503 | Vertex declined to return presentation content under its content-safety policy.               |
| `BEDROCK_AUTHENTICATION_FAILED`         |         503 | AWS rejected the runtime role or credentials.                                                 |
| `BEDROCK_INVALID_REQUEST`               |         503 | Bedrock rejected the adapter payload or selected model contract.                              |
...
| `*_OUTPUT_TRUNCATED`                    |         503 | The provider stopped at its output-token ceiling before the structured response was complete. |
